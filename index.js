...
console.log("Görev 1. ", fenomenler[0].profile);
...
function indekseGoreFenomen(liste,index) {
  
 let fenomen = fenomenler[3].profile
return index + ". indekste bulunan fenomen: " + fenomenler[3].profile;
...
function profilListesi(liste) {
  const yeniliste = liste;
  return liste.map((liste) => liste.profile);             //.map() işlevini kullanara dize içerisinde istediğimiz bilgiyi alabilriliz.
...
function fenomenSil(liste,index) {
const yeniliste2 = liste; 
yeniliste2.splice(index,1); 
return yeniliste2;
...
function fenomenEkle(liste,number,profile,followers,posts,platform) {
 const copylist = liste;
copylist.push(inNesne);
return copylist;
...
function enFenomenler(liste) {
  const uplist = [];
for (i = 0 ; i < liste.length ; i++) {
  if (liste[i].followers > 100000000 ) {
    uplist.push(liste[i].profile);
  } 
}    return uplist;
...
function fenomenGonderimSayisi(liste,name){
for (i = 0 ; i < liste.length ; i++) {
if (liste[i].profile.includes(name)){
return liste[i].posts;
}
}
...
function platformaGoreCokGonderiYapanFenomen(list,app){
  let maxPost = 0;
  let maxPostFen = "";

  for (let i = 0; i < list.length; i++) {                       // Dizin içerisinde baştan sona kontrol ****
                                                                      
    if (list[i].platform === app && list[i].posts !== "NA") {            // if kullanara sorguladık.
      if (list[i].posts > maxPost) {                                    
        maxPost = list[i].posts;                                          
        maxPostFen = list[i].profile;
      }
    }
  }
